...
def soup_to_listings_adapter(soup):
    ...
    for listing in listing_soups:
        ...
        price = listing.find_all('span', 'listingCard-priceMargin')[0].text.strip()
        area = listing.find_all('p', 'listingCard-upperShortLabel')[0].text.strip()
        num_beds = listing.find_all('div', 'listingDetailDefinitions')[0].find_all('span')[1].text
        ...
        listings.append(RentalListing(address, area, price, net_effective_rent, num_beds, has_fee))
